...
An example of such a potential is shown in Fig X. %TODO 
...
Motor friction & $\gamma_{m}$ & 0.66 & pN $\upmu$m/nm & REF\\
Actin friction & $\gamma_{a}$ & 0.97 & pN $\upmu$m/nm & REF\\
\end{tabular}
...
= \frac{k_a k_d}{k_a + k_d}\left[\Delta E - (1-c)\langle V_r(x) \rangle_{s=1} \right] \label{Q_in} \\
\langle \mathcal Q_{out} \rangle 
= -\rho^{eq}_0 k_a \left[ \Delta E - (1-c)\langle V_r(x) \rangle_{s=0} \right] 
= -\frac{k_a k_d}{k_a + k_d} \left[ \Delta E - (1-c)\langle V_r(x) \rangle_{s=0} \right] \label{Q_out}
\end{gather}
...
When the load is increased to $F_l = 25 \, \mathrm{pN}$, the distributions shift to the right but distribution for the attached state remains peaked around the potential minimum, while the detached distribution flattes out and increases around the potential maximum. 
...
= \int_0^d V_r(x) \, \delta\rho_1 \; \rmd x\propto \int_0^d V_r(x) \, \partial_x V_r(x) \; \rmd x 
...
= \int_0^d \left\{ \frac{1}{d} + \frac{1}{ d F } \partial_x V_r + \frac{1}{ d F^2 } \left[ \left( \partial_x V_r \right)^2 - k_B T \partial_x^2 V_r \right] + C^\prime\right\} \; \rmd x 
\end{equation}
Which simplifies to
\begin{equation}
...
\begin{equation}
\rho_2 = \rho_0 + \delta\rho_1 + \delta\rho_2 = \frac{1}{d} + \frac{1}{d F} \partial_x V_r 
...
&= \frac{1}{d F^2} \int_0^d V_r(x) \left[ \left(\partial_x V_r \right)^2 - \frac{1}{d} \int_0^d \left( \partial_y V_r \right)^2 \; \rmd y  - k_B T \partial_x^2 V_r \right] \; \rmd x
\end{align*}
