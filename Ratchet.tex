...
We can define the internal energy stored in the motor heads as the sum over the potential energies of all heads, 
which depends on the position of the motor relative to the actin filament and the states of all heads.
\begin{equation}
E(x_\text{M}-x_\text{A}, \{s\}) = \sum_{i=1}^N (1-s(i))\Delta E + \zeta(i)V_r(x_\text{M} - x_\text{A} -id)
\end{equation}
This quantity will fluctuate due to the diffusion of the motor and actin filament and it will change abruptly when a head changes its chemical state.
\begin{align}
\dot{E} =& \frac{\partial E}{\partial x_\text{M}}\dot{x_\text{M}} + \frac{\partial E}{\partial x_\text{A}}\dot{x_\text{A}} + \sum_{i=1}^{N}\frac{\partial E}{\partial s(i)}\dot{s(i)}\\
=& \nabla_{\text{M}} V \dot{x_\text{M}} + \nabla_{\text{M}} V \dot{x_\text{A}} + \dot{\mathcal Q}_\text{in} + \dot{\mathcal Q}_\text{out}\\
=& -\left(\dot{\mathcal W}_\text{M} + \dot{\mathcal W}_\text{A} \right) + \dot{\mathcal Q}_{\text{in}} + \dot{\mathcal Q}_{\text{out}}
\end{align}
Here, we can identify currents of the work done by the ratchet force on the motor $\mathcal W_\text{M}$ and on the actin filament $\mathcal W_\text{A}$,
 as well as the heat exchanged with the chemical cycle $\mathcal Q_\text{in}$ and $\mathcal Q_\text{out}$. %TODO include figures and explain that since E is confined, change in E is zero and total change in chemical heat is equal to work done by ratchet force
...
\begin{align}
\mathcal Q_\text{in} &= \sum_{t_d \leq T} \left[ \Delta E + (c-1) V_r(x_{t_d}) \right]  \label{q_in} \\
\mathcal Q_\text{out} &= -\sum_{t_a \leq T} \left[ \Delta E + (c-1) V_r(x_{t_a}) \right]  \label{q_out}
\end{align}
...
Here, we are interested in the force-velocity relation of the filaments, rather than the motor.
...
\begin{align}
\langle v_{A1} \rangle &= \frac{1}{ \gamma_A} \left( -F_{load} - F_{r1}  \right) \\
\langle v_M \rangle &= \frac{1}{\gamma_M } \left( F_{r1} + F_{r2} \right)\\
\langle v_{A2} \rangle &= \frac{1}{ \gamma_A} \left( F_{load} - F_{r2}  \right) 
\end{align}
where $A1$ and $A2$ denote the two different actin filaments and $F_{r1}$ and $F_{r2}$ are the ratchet forces between the motor and respectively filament $A1$ and $A2$. 
Note that the mean velocity of the motor is proportional to the mean center of mass velocity of both actin filaments. 

\begin{equation}
\langle v_M \rangle = \frac{\gamma_A}{\gamma_M} \left(\langle v_A1 \rangle + \langle v_A2 \rangle \right) \label{tug_F_motor}
\end{equation}
...
The force-velocity relation of the filaments is shown in figure \ref{Fig: tug_F}.
\begin{figure}[t]
\centering
