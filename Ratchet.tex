...
\label{sec:ratchet}
We consider a one dimensional model representing a myosin motor interaction with an actin filament, see figure~\ref{Fig: ratchet setup}.
...
These simplifications allow us to model both, the motor and the filament, as point particles undergoing overdamped diffusion. 
...
\label{eom_A}
\end{align}
where $x_\text{M}$ ($x_\text{A}$) denote the position of myosin motor (actin polymer), 
...
\begin{equation}
V_t(x) = \sum\limits_{i=1}^{N} \zeta_t(i) \, V_\text{r} (x - i \, d ), 
\end{equation}
where $d$ is the distance between myosin heads, 
$\zeta_t(i)$ represents the internal state, see bellow, 
and $V_\text{r}$ is the periodical ratchet potential 
\begin{equation}
