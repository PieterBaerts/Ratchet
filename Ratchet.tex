...
The transition between ATP \emph{bounded} and \emph{unbounded} state is govern by rates $k_\text{bind}$, $k_\text{unbind}$ 
and is associated with the chemical energy $\Delta E$.
In the \emph{bounded} state the ratchet potential is scaled down by factor $c$. 
}
\label{Fig: energy}
\end{figure}
...
\begin{align}
\langle v_A1 \rangle &= \frac{1}{ \gamma_A} \left( -F_{load} - F_{M1}  \right) \\
\langle v_M \rangle &= \frac{1}{\gamma_M } \left( F_{M1} + F_{M2} \right)\\
\langle v_A2 \rangle &= \frac{1}{ \gamma_A} \left( F_{load} - F_{M2}  \right) 
\end{align}
