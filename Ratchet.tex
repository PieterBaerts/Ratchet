...
For a system in the simple setup which have reached already a steady state the mean heat \eqref{eq:q_in} and \eqref{eq:q_out} produced over a time interval of length $\Delta t$ 
by transitions of a single head $i$ displaced by $i d$ is given by 
\begin{gather*}
\begin{multlined}[b][.42\textwidth]
...
By using again the independence of the evolution of individual heads we can express the total mean heats produced in chemical cycle as a sum of contributions from individual heads
\begin{align*}
...
\begin{equation}
\eta_\text{chem} = (1-c) \sum\limits_{i=0}^{N-1} \frac{ \left\langle V_r(x-id) \middle| \zeta_i = c \right\rangle - \left\langle V_r( x -id ) \middle| \zeta_i = 1 \right\rangle }
{ N \Delta E - (1-c) \sum\limits_{j=0}^{N-1} \left\langle V_r(x-jd) \middle| \zeta_j = 1 \right\rangle } 
. 
\label{eq:eta}
\end{equation}
This efficiency is shown in figure~\ref{fig:chem}.
\begin{figure}[t]
\centering
