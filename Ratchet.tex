...
\begin{description}
\item[Continuity] The chemical rates do not depend on spacial degrees of freedom and motor heads can attach and detach anywhere along the polymer filament.
\item[Independence of ATP cycle] The chemical rates do not dependent on external forces on the motor but solely on concentrations of chemicals involved in ATP hydrolysis. We assume that constant concentrations are maintained in the environment.
\end{description} 
Furthermore we will be studying non-muscle myosin II motors, that have a small number of heads \cite{}. %TODO 
...
These simplifications allow us to model both the motor and the filament as point particles undergoing overdamped diffusion \cite{vanKampen1981stochastic}.
...
Here, the internal state represents whether ATP, is \emph{bound} or \emph{unbound} to the myosin head,
not whether the head itself is bound or unbound to a given actin filament. %TODO
...
As polymers inside the cytoskeleton are usually thoroughly interconnected rather then freely diffusing \cite{blanchoin2014actin,ennomani2015architecture},
...
\caption{
\label{fig:v_k} 
Mean velocity of the motor relative to the actin filament $\langle v \rangle$ for varying $k_\text{unbind}$.
...
\caption{
\label{fig:tug_F}
Analogue to figure~\ref{fig:ratchet_force}, now with two filaments instead of one and an external force in the filaments instead of the motor.
}
\end{figure}
...
If we interpret the dependency of the chemical efficiency on the load as a probabilistic distribution with finite variance and assume that the main contribution to the shape, figure~\ref{fig:chem}, is from the numerator, 
...
From the figure \ref{fig:chem_eff_1head} it is also apparent that there is no qualitative difference in chemical efficiency between the system with four heads and one head,
hence we will further restrict the discussion to the more simple case of system having only a single head.
The chemical efficiency expression then simplifies to
\[
\eta_\text{chem} = (1-c)
\frac{ \left\langle V_r(x) \middle| \zeta = c \right\rangle - \left\langle V_r(x) \middle| \zeta = 1 \right\rangle }
{ \Delta E - (1-c) \left\langle V_r(x) \middle| \zeta = 1 \right\rangle } 
. 
\] 
As the potential is known in our model, the only missing information is thus the steady state distribution. 
These distributions are empirically obtained by collecting the positions of the heads over all times that the head was in the attached/detached state. 
...
Since the relaxation of position in a given potential is fast, compared to the chemical rates, this approximates the steady distribution very well.
...
Density of the nett energy output \eqref{} produced by the chemical cycle for various load forces $F_\text{load}$. %TODO
