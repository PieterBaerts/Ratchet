...
The force-velocity relation can also be exploited to derive the tension in the motor. Since both the load and ratchet force are directly applied to the motor, the tension can be derived from equations (\ref{pre_velocity_M}),(\ref{pre_velocity_A}), (\ref{velocity_M}) and (\ref{velocity_A}).
\begin{align}
...
And by the work caused by the external load 
\[
{\mathcal W}^\text{ext}_\text{M} = \int \rmd x_\text{M} \circ F_\text{load} = \Delta x_\text{M} F_\text{load}, 
\]
where we used the fact that the external load $F-\text{load}$ is constant. 

The last term in the energy balance \eqref{eq:energy_balance} is the contribution from the chemical cycle. 
The chemical cycle consists of binding an ATP, it's hydrolyses, release of phosphor and finally release of ADP, 
...
Note that, when we use the equation of motion \eqref{eq:eom_M} the heat exchange with the surrounding \eqref{eq:heat_hb_M} can be rewritten as 
\begin{multline*}
{\mathcal Q}_\text{M}^\text{hb} 
...
The first efficiency discussed in details in \cite{} %TODO 
...
This will allow us to use steady state distributions to calculate the expected values of the potential energy in equation \eqref{eta}.
...
As we want to calculate perturbations for large loads we will rewrite equation \eqref{FP_ss} so that we can use $\epsilon = \frac{1}{F}$ as the perturbation.
\begin{equation}
...
When inserted into \eqref{FP_ss_pert} and only keeping terms up to first order in $\frac{1}{F}$, we get
\begin{equation*}
...
Now by inserting $\rho_2 = \rho_1 + \delta\rho_2$ in \eqref{FP_ss_pert} and only keeping up to second order in $\frac{1}{F}$, we find
\begin{multline*}
...
\end{align*}
The first two terms will cancel each other and after carrying out the integral for the third term, with $V_r(x)$ given by \eqref{rat_pot}, one finds 
\begin{equation}
...
The current is taken from the Fokker-Planck equation \eqref{FP_ss}.
\begin{equation}
...
The understand this we must again look at equation \eqref{eta} which is determined by the ratchet potential averaged with the heads' distribution in the attached and detached state. 
...
\end{equation*}
The tension on the motor is shown in figure \ref{fig:tug_F}, where it is compared to the tension when the load is applied on the motor instead of the filaments, given by \eqref{tension}. 
\begin{align*}
