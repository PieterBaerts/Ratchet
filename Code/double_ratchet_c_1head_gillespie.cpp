...
int main(int argc, char* argv[]){
	...
	printf("#c\tv(nm/us)\tdv\tQ_in\tQ_out\tdQ_in\tdQ_out\n");
        for(int i = 0; i < N_a; i++){
//                double a = a_min + (a_max-a_min)*i/double(N_a-1);
                for(int j = 0; j < N_c; j++){                        
                        double c = c_min + (c_max - c_min)*j/(N_c-1);
//       			if( j != 0)
//				F = exp( log( F ) + 0.06  );
			...
			Q_out2 /= double(N);
                    	...
                    	printf("%g\t%g\t%g\t%g\t%g\t%g\t%g\n", c, v_tot, sqrt(v2_tot - v_tot * v_tot), Q_in, Q_out, sqrt(Q_in2 - Q_in*Q_in), sqrt(Q_out2 - Q_out*Q_out));
                    	fflush(stdout);
