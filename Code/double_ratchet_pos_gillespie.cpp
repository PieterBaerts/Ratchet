...
double speed_sim(const int N_heads, const double F_stall, const double a, const double Vmax, double c, double &Q_in, double &Q_out);
...
//                        double F = F_min + (F_max - F_min) * j / double(N_F-1);
			...
			double Q_in = 0, Q_out = 0;
			            for(int k=0; k<N; k++){
		                    double v = speed_sim(N_heads, F, a, V, c, Q_in, Q_out);
		                    v_tot += v;
			                v2_tot += v * v;
			            }
...
//                    	printf("%lf\t%lf\t%lf\t%lf\t%lf\n", F, v_tot, sqrt(v2_tot - v_tot * v_tot), Q_in, Q_out);
...
double speed_sim(const int N_heads, const double F_stall, const double a, const double Vmax, const double c, double &Q_in, double &Q_out){
	...
	for(int i=0; i < N_timesteps; i++, t+=dt){
	    ...
	    if(i%printevery == 0){
                printf("%lf\t", i*dt);
		for(int j=0; j < N_heads; j++){   
   		    printf("%lf\t%d\t", motor_x - j*l_heads - actin_x - l*floor((motor_x - j*l_heads - actin_x)/l), motor_bound[j]);		
		}
		printf("\n");
            }        
	}
