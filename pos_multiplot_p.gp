...
set output "pos_multiplot_p.pdf" 
...
f = "0_attached 0_detached 25_attached 25_detached 100_attached 100_detached 250_attached 250_detached"
val = "0 0 25 25 100 100 250 250"
state = "unbound bound unbound bound unbound bound unbound bound unbound bound unbound bound unbound bound"
color = "1 10 3 12 5 14 7 16"

plot [0:8][0:0.4] \
      for [i=1:8] sprintf("Data/pos_distr_F=%s.dat", word(f,i)) u (shift($1+6)):($2*10) title sprintf("F_{load}(%s) = %s pN", word(state,i), word(val,i)) w points ls word(color,i) \
