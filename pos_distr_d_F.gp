...
set output "pos_distr_d_F.pdf" 

load "Data/my.pal"

set xlabel "x (nm)"
set ylabel "~{/Symbol r}\342\200\276 (x|{/Symbol z}=c)"
...
plot [0:8][0:0.4] \
      for [i=1:5] sprintf("Data/pos_distr_F=%s_detached.dat", word(frev,i)) u (shift($1+6)):($2*10./(1./3.)) title sprintf("F_{load} = %s pN", word(f,i)) w points ls word(color,i) \
